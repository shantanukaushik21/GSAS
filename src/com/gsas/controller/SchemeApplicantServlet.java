...
/**
 * Servlet implementation class SchemeApplicantServlet
 */
@WebServlet("/SchemeApplicantServlet")
public class SchemeApplicantServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * Default constructor. 
     */
    public SchemeApplicantServlet() {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ApplicantService schemeApplicantService = (ApplicantService) ObjectFactory.getInstance(LayerType.SCHEME_APPLICANT_SERVICE);
		...
		if(loginVO != null) {
			//long schemeId = ; code for getting scheme id
			...
			CitizenDetailsVO citizenDetails = citizenService.getCitizenDetails(loginVO.getCitizenId()); //get citizenDetails from citizenId
			...
			Boolean status = schemeApplicantService.validate(schemeEligibilityVO, citizenDetails, gender, age);
			...
			if(status == true) {
				...
				schemeApplicant = schemeApplicantService.validate(schemeVO, bank, docList, schemeApplicant);
				if(schemeApplicant.isApprovedStatus()){   //check if status = true after doc and bank validation.
					schemeApplicantService.addSchemeApplication(schemeApplicant, docList); // fill scheme_applicant table with status=true
				} //and add document path in scheme_applicant_document table
				else {
					schemeApplicantService.addSchemeApplication(schemeApplicant);
